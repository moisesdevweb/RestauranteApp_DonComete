...
export function useReportes() {
    ...
    try {
      ...
      if (v === 'semanal') {
        const [s, top] = await Promise.all([
          getReporteSemanal(fechaHoy),
          getProductosTop(fechaHoy, fechaHoy), // el backend calcula el rango
        ]);
        setSemanal(s);
        setProductosTop(top);
      }
  ...
  }, [fecha, año, mes, fechaHoy]);

  useEffect(() => { cargar(vista); }, [vista, fecha, año, mes]);
